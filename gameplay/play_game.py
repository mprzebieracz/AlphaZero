...
class Connect4GUI:
    def __init__(self, root, game, red: Agent, yellow: Agent):
        self.root = root
        self.game = game
        self.red_agent = red
        self.yellow_agent = yellow
        self.canvas = tk.Canvas(root, width=COLS*CELL_SIZE, height=ROWS*CELL_SIZE + 50, bg='white')
        self.canvas.pack()
    ...
    def draw_board(self):
        ...
        self.canvas.create_text(CELL_SIZE * COLS // 2, ROWS * CELL_SIZE + 20,
    ...
    def play_turn(self):
        if self.game.done:
            ...
            self.canvas.create_text(CELL_SIZE * COLS // 2, ROWS * CELL_SIZE + 20,
                                    text=winner_text[self.game.winner], font=("Arial", 18, 'bold'), fill='green')
            return
