...
class DynamicBatcher {
    ...
    execute_tensor_batch(torch::Tensor batched, const std::vector<int> &legal_actions_flat,
        ...
        try {
            ...
            std::cerr << "Exception caught in execute_tensor_batch: " << e.what() << '\n';
            throw;
        }
        return out;
    }
  ...
  public:
    DynamicBatcher(int wait_for_count, int timeout_ms, std::shared_ptr<Network> network,
        ...
        : wait_for_count(wait_for_count), timeout_ms(timeout_ms), network(network),
          infer_method(network->find_method("forward")
                           ? network->get_method("forward")
                           : network->get_method("infer")),
          device(device) {
...
std::vector<inference_result> NetworkInferer::infer(const std::vector<const GameState *> &states) {
    if (states.empty())
        return {};
...
static std::shared_ptr<Network> get_network_func(std::string network_file_path,
    ...
    if (std::filesystem::exists(network_file_path)) {
        try {
            ...
            static const bool trt_runtime_loaded = []() {
                ...
                if (handle == nullptr) {
                    ...
                    std::cerr << "Failed to pre-load libtorchtrt from '" << ALPHAZERO_TRT_LIB_PATH
            ...
            std::cerr << "Failed to load network from " << network_file_path
                      << ". Ensure it is exported using TorchScript.\n";
        ...
        std::cerr << "File " << network_file_path << " doesn't exist\n";
...
static void enable_cudagraphs_if_requested(torch::Device device) {
    ...
    if (!op.has_value()) {
        std::cerr << "tensorrt::set_cudagraphs_mode op not found; CUDAGraphs mode not enabled "
                     "(is libtorchtrt.so actually loaded?)\n";
        return;
    }
    op->typed<void(int64_t)>().call(kSubgraphCudagraphs);
    std::cout << "CUDAGraphs mode enabled for TensorRT inference (ALPHAZERO_ENABLE_CUDAGRAPHS set)\n";
}
#endif
