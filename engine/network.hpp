...
struct ResidualBlockImpl : torch::nn::Module {
    ...
    ResidualBlockImpl(int channels) {
        conv1 = register_module(
            "conv1",
            torch::nn::Conv2d(torch::nn::Conv2dOptions(channels, channels,
                                                       /*kernel_size=*/3)
                                  .stride(1)
                                  .padding(1)
                                  .bias(false)));
...
struct AlphaZeroNetworkImpl : torch::nn::Module {
    ...
    AlphaZeroNetworkImpl(int input_channels, int h, int w, int num_res_blocks,
        ...
        conv_in = register_module(
            "conv_in", torch::nn::Conv2d(torch::nn::Conv2dOptions(
                                             input_channels, num_filters, 3)
                                             .stride(1)
                                             .padding(1)
                                             .bias(false)));
        ...
        value_fc1 = register_module(
            "value_fc1", torch::nn::Linear(height * width, num_filters));
        value_fc2 =
            register_module("value_fc2", torch::nn::Linear(num_filters, 1));
    }
    ...
    //         std::cerr << "Error loading the model weights from " << path <<
    //         ": "
    //                   << e.what() << std::endl;
    //         // handle error or rethrow if you want
    //     }
    // }
};
