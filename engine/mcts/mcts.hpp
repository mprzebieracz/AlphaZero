...
struct Inferer {
        // Inferer should have a method to predict the policy and value for a given game state
...
struct Node {
    ...
    float UCB(float exploration_weight) const {
        if (!parent) return 0;
        return Q() + exploration_weight * prior * std::sqrt(parent->visits) / (1 + visits);
    }

    void expand(const std::vector<float>& policy) {
        for (int i = 0; i < policy.size(); i++) {
            if (policy[i] == 0.0f) continue;
            auto child_state = game_state->clone();
            child_state->step(i);
            children[i] = std::make_unique<Node>(std::move(child_state), policy[i], this);
        }
        expanded = true;
    }

    bool isTerminal() const {
        return game_state->isTerminal();
    }
    ...
    std::pair<int, Node*> selectChild(float exploration_weight) {
        int best_index = -1;
...
public:
    MCTS(
        ...
        float eps = 0.25f,
        float alpha = 0.3f
    ) : network(network), c_init(c_init), c_base(c_base), eps(eps), alpha(alpha), device(network.device) {}

    std::vector<float> search(const Game& game, int num_simulations = 800) {
        auto root = game.clone();
        Node root_node(std::move(root));
        
        auto [p_init, _] = getPolicyValue(game, true);
        ...
        for (int sim = 0; sim < num_simulations; sim++) {
            ...
            float value;
            if (!node->isTerminal()) {
                auto [p, v] = getPolicyValue(*node->game_state, false);
                node->expand(p);
                value = v;
            }
            else value = node->game_state->reward();
        ...
        if (sum > 0.0f) for (auto &x : pi) x /= sum;
        return pi;
    }
...
private:
    std::pair<std::vector<float>, float> getPolicyValue(const Game& game, bool dirichletNoise = false) {
        ...
        for (int a : legal) mask_vec[a] = 1.0f;
        ...
        if (sum > 0.0f) {
            ...
            for (int a : legal) policy_vec[a] = inv;
        }
