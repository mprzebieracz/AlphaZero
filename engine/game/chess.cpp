...
void Chess::move_rules_k(PosList &moves) const {
    ...
    // Castling: king must not be in check and must not pass through an attacked
    // square. (The destination square is checked by actions()'s simulation.)
    auto through_safe = [&](int col) {
        auto copy = current_board;
        copy[0][4] = EMPTY;
        copy[0][col] = B_KING;
        return !bitboard::is_attacked(1, copy);
    };
    if (can_castle(0) && !check_status() && through_safe(3))
        moves.emplace_back(0, 2);
    if (can_castle(1) && !check_status() && through_safe(5))
...
void Chess::move_piece(int r1, int c1, int r2, int c2, int promoted_piece) {
    if (player == 0) {
        ...
        if (piece == W_KING)
            K_move_count++;
        if (piece == W_KING && r1 == 7 && c1 == 4 && std::abs(c2 - c1) == 2) {
            // Castling: also move the rook.
            if (c2 == 6) {
                current_board[7][5] = current_board[7][7];
                current_board[7][7] = EMPTY;
                R2_move_count++;
            } else {
                current_board[7][3] = current_board[7][0];
        ...
        if (piece == B_KING)
            k_move_count++;
        if (piece == B_KING && r1 == 0 && c1 == 4 && std::abs(c2 - c1) == 2) {
            // Castling: also move the rook.
            if (c2 == 6) {
                current_board[0][5] = current_board[0][7];
                current_board[0][7] = EMPTY;
                r2_move_count++;
            } else {
                current_board[0][3] = current_board[0][0];
            ...
            for (int j = 0; j < 8; ++j) {
                ...
                if (is_white(p)) {
                    ...
                    case W_ROOK:
                        ...
                        break;
                    case W_KNIGHT:
                        move_rules_N(i, j, moves);
                        break;
                    case W_BISHOP:
                        move_rules_B(i, j, moves);
                        break;
                    case W_QUEEN:
                        move_rules_Q(i, j, moves);
                        break;
                    case W_KING:
                        move_rules_K(moves);
                        break;
                    case W_PAWN:
                        move_rules_P(i, j, moves);
                        break;
                    default:
                        std::abort();
                    }
